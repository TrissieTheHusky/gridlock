...
do
  ...
  do
    ...
    do
      rand=$[ $RANDOM % 3 ]
      DATE=`date +%s`
      curl -q -i -H "Content-Type: application/json" -X PUT -d '{"description":"'${service}'", "service":"'${service}'","status":"'${arr[$rand]}'","location":"'${loc}'","env":"prd","timestamp":"'${DATE}'"}' http://localhost:5000/gridlock/api/v0.1/
    done
  done
sleep 60
done
